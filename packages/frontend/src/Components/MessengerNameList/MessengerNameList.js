...
const MessengerNameList = ({ rooms, currentSelected, updateSelected, listReference }) => {
  return (
    <div className={styles.MessengerNameList} ref={listReference} >
      {rooms.map((room, i) => {
        return <NameCard 
          key={i} 
          {...room} 
          isSelected={currentSelected === room.id} 
